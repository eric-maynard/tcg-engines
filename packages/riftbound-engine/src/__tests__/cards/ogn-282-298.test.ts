 ...
 * (no buff → cannot spend), 702.2.b.2 (only buffs on units YOU control), 355.10.c.1 ("spend a
 * buff to draw 1" is a cost-within-instruction: unpayable → no draw; the buff is not a target).
 *
...
describe("Monastery of Hirana (ogn-282-298)", () => {
  test("registry payload: one optional 'conquer here' trigger whose effect is spend-buff → then draw 1", async () => {
    const def = (await loadDefaultCardPool()).get(CARD);
    expect(def).toMatchObject({ cardType: "battlefield", name: "Monastery of Hirana" });
    expect(def?.abilities).toEqual([
      {
        effect: { then: { amount: 1, type: "draw" }, type: "spend-buff" },
        optional: true,
  ...
  test("the trigger is a real chain item: P2 gets priority to respond before it resolves, and nothing is drawn until it does", async () => {
    const game = await siege().unit(P1, "base", { might: 3, name: "Monk" }, "monk", { buffed: true }).build();
    await game.p1.move("monk", "mon");
    await game.settle();
    await game.p1.yes();
    // After opting in, the item waits on the chain for both passes.
    expect(game.chain()).toHaveLength(1);
    expect(game.p1.hand()).toHaveLength(0);
    await game.p1.passPriority();
    expect(game.decision()).toMatchObject({ context: "chain", kind: "action", seat: P2 });
    expect(game.state("monk").isBuffed).toBe(true); // cost-within-instruction is paid on resolution
