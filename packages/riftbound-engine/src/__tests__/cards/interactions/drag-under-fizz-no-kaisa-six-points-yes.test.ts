 ...
 * Under stays put. (b) eligible (5 < 6); offered from the trash; Energy 0, exactly [order] vs E,
 * [order]+[A] vs F; the −2 does nothing; played as P1's chain item mid-resolution regardless of its
 * Action tag, target chosen at play time, P2 gets a Reaction window. DESIGN (DESIGN.md §Paying costs):
 * paying is manual/pool-only — F is offered only once the extra [A] is IN the pool; the play prompt
 * keeps rune actions open so P1 recycles first. (c) bottom of P1's main deck, E dead; countered →
 * still recycled, E alive, [order] gone. (d) 5 < 5 false → never offered. (e) 5+[order] from hand
...
/** …P1 accepted and everyone passed: the trigger resolves and P1 is told to play the trash spell (the offer is open). */
async function offered(opts: KaisaOpts = {}): Promise<Game> {
  const game = await conquered(opts);
  await game.p1.yes();
  for (let i = 0; i < 6 && !offersDrag(game.decision()); i++) {
    const r = await game.settle();
    if (r.reason === "open") {
      break;
    }
  }
  expect(offersDrag(game.decision())).toBe(true);
  return game;
}
...
describe("Drag Under from the trash — Fizz (no) vs Kai'Sa at six points (yes)", () => {
  ...
  test("(b) 6 points (5 + the conquer): printed 5 < 6 → after accepting Kai'Sa's trigger P1 is offered exactly Drag Under out of the trash", async () => {
    const game = await offered();
    const d = game.decision();
    expect(d?.kind === "pick" ? d.options.map((o) => o.card ?? o.key) : []).toEqual(["drag"]);
    expect(game.zoneOf("drag")).toBe("trash"); // still there until P1 takes the offer
  });

  // BUG — expected (355.10.a, 355.5): the trash is a PUBLIC zone, so "a spell from your trash" is a
  // TARGET of Kai'Sa's trigger — named at finalization, right after the "you may", while the trigger is
  // still on the chain for both players to respond to (as the engine already does for Fizz). Actual:
  // Kai'Sa's trigger is finalized with no target and the spell is only picked as it resolves.
  test.failing("BUG: (b) the trash spell is a TARGET of the conquer trigger — P1 names Drag Under at finalization and Kai'Sa's chain item carries it before anyone gets priority (355.10.a)", async () => {
  ...
  test("(b) cost vs E: Energy is not paid at all (0 stays 0 — the '−2 elsewhere' has nothing to reduce and never rebates) and exactly the [order] Power is debited the moment P1 takes the play (356.1.b.2, 356.6)", async () => {
    const game = await offered({ order: 1 });
  ...
  test("(b) vs [Deflect] F the mandatory extra [A] is owed (356.2.a.2, 809): with only [order] banked F is NOT a legal choice; with [order]+[fury] banked it is, and choosing it spends both", async () => {
    const poor = await offered({ order: 1 });
    await poor.p1.pick("drag");
    const dp = poor.decision();
    expect(isDragTargetPick(dp) ? dp.options.map((o) => o.card ?? o.key) : []).not.toContain("F");

    const rich = await offered({ fury: 1, order: 1 });
    await rich.p1.pick("drag");
  ...
  // MANUAL. The "play it" offer keeps P1's rune actions open, so P1 recycles a rune for the [A] first
  // and only then takes the play — after which the Deflect unit is a legal, payable choice.
  test("(b) [Add] while paying, engine-style: at the play offer P1 recycles a fury rune (+[fury]) — then, and only then, F joins the legal targets and can be paid for", async () => {
    const game = await offered({ order: 1, runes: 1 });
    ...
    expect(offersDrag(game.decision())).toBe(true); // still the same offer
    await game.p1.pick("drag");
    const d = game.decision();
  ...
  test("(d) exactly 5 points (4 + the conquer): 'less than your points' — 5 < 5 is false → Drag Under is never offered; whatever P1 answers it stays in the trash, E lives, the [order] is unspent", async () => {
    const game = await conquered({ order: 1, points: 4 });
