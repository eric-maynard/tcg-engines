...
describe("Sandswept Tomb (ven-164-166)", () => {
  ...
  test("a spell choosing a friendly unit here costs [rainbow] less — Primal Strength on Mine Here pays 4 energy and keeps the body power", async () => {
  ...
  test("target-dependent affordability — 4 energy, 0 power: castable, and only onto the friendly unit here", async () => {
  ...
  test("'each spell' — the opponent's spell on THEIR unit here is discounted too, whoever controls the Tomb", async () => {
  ...
  test("a two-role spell (Challenge) whose FRIENDLY choice is here is discounted once — 2 energy, body power kept", async () => {
    const game = await scenario()
  ...
  test("a two-pip spell choosing a friendly unit here still owes exactly one pip (3 energy + 1 power suffices)", async () => {
    const game = await scenario()
  ...
  test("registry payload — a static [rainbow] cost-reduction for spells choosing friendly units here (currently no abilities at all)", async () => {
