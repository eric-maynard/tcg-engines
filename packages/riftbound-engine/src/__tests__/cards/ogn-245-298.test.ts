/**
 * Seal of Unity — ogn-245-298 · Gear · Order · 0 energy + [order]
 *
 *   [Exhaust]: [Reaction] — [Add] [order].
 *   (Abilities that add resources can't be reacted to.)
 *
 * Add abilities resolve immediately on finalization — they never sit on the chain.
 * [Reaction] timing lets it be used on the opponent's turn / with priority on a chain.
 */
...
const CARD = "ogn-245-298";
const DISINTEGRATE = "ogn-005-298"; // [Action] 4 energy: deal 3 to a unit at a battlefield

describe("Seal of Unity (ogn-245-298)", () => {
  test("costs 0 energy + 1 order to play; lands in base as gear; unaffordable without the order power", async () => {
    const game = await scenario().resources(P1, { energy: 0, power: { order: 1 } }).hand(P1, CARD, "seal").build();
    ...
    const wrongPower = await scenario().resources(P1, { energy: 0, power: { fury: 1 } }).hand(P1, CARD, "seal").build();
    expect(wrongPower.p1.can("equip", "seal")).toBe(false);
  });

  test("[Exhaust]: adds 1 order power immediately — nothing goes on the chain, the gear becomes exhausted", async () => {
  ...
  test("the exhaust cost gates it: cannot be activated while exhausted; readies at your next Awaken", async () => {
  ...
  test("[Reaction] timing: usable on the opponent's turn and in response to an opponent's spell, without joining the chain", async () => {
  ...
  test("the added order power can pay an [order] cost in the same turn", async () => {
  ...
  test("unspent added power empties from the pool at end of turn", async () => {
    const game = await scenario().gear(P1, CARD, "seal").build();
    await game.p1.activate("seal");
    expect(game.p1.power("order")).toBe(1);
    await game.advanceTurn();
