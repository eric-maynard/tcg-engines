 ...
 * Rules: 719 ([Assault] applies while the unit is an attacker), 466 (roles end with combat, then the
 *        conquer is settled), [Mighty] = 5+ Might.
 */
...
describe("Ruling fdb0fceec74e6ef1 — [Assault] does not make a unit Mighty for Sunken Temple's conquer trigger", () => {
  ...
  // Engine: the combat role is still "attacker" when the conquer trigger is finalized, so the raider
  // reads 5 Might, counts as [Mighty], and Sunken Temple offers its "pay [1] to draw 1" anyway.
  test.failing(
    "BUG: ruling fdb0fceec74e6ef1 — Sunken Temple must NOT trigger for a 3-Might [Assault 2] raider; the engine keeps the attacker role (raider reads 5 Might) and offers the draw",
    async () => {
      const game = await board(ASSAULT_RAIDER).build();
      await conquer(game);
      expect(game.state("raider").might).toBe(3); // designation removed before the conquer is settled
