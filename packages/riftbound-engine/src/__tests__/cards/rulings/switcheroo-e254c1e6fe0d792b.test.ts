/**
 * Ruling e254c1e6fe0d792b — Switcheroo (sfd-145-221) · Action spell · Chaos · 2 + [chaos][chaos]
 ...
 * (+3) and P2's 1-Might "weakling". P1 has 2 + [chaos][chaos] for Switcheroo plus 2 more energy for Discipline.
 */
function board() {
  return scenario()
...
describe("Ruling e254c1e6fe0d792b — Switcheroo swaps CURRENT Might; existing bonuses are not re-counted", () => {
  ...
  test("ruling e254c1e6fe0d792b — Switcheroo on [5-Might equipped bearer, 1-Might weakling] → bearer 1, weakling 5; the equipment's +3 is not re-added", async () => {
    const game = await board().build();
  ...
  test("ruling e254c1e6fe0d792b — a NEW +2 (Discipline) after Switcheroo simply stacks: bearer 1 → 3, weakling still 5", async () => {
    const game = await board().build();
    await game.p1.cast("switcheroo", { targets: ["bearer", "weakling"] });
    await game.settle();
  ...
  test("ruling e254c1e6fe0d792b — the swap expires at end of turn: bearer back to 5 (sword still counted once), weakling back to 1", async () => {
    const game = await board().build();
    await game.p1.cast("switcheroo", { targets: ["bearer", "weakling"] });
    await game.settle();
    expect(game.state("bearer").might).toBe(1);
