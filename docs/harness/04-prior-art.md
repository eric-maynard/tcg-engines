...
| Replay/undo | `getReplayHistory()`, `replay(upTo?)`, `undo()/redo()` via Immer inverse patches + internalState snapshots; `getPatches/applyPatches` for net sync | `rule-engine.ts:1169-1349` |
...
- `undo()` restores `currentState` + `internalState` only — not FlowManager phase/turn, TrackerSystem, or `gameEnded` (`rule-engine.ts:1223-1247`). The core guide's minimax-via-undo example (`packages/core/docs/guides/move-enumeration.md:521-560`) is therefore unsafe once moves trigger flow transitions.
...
- For search/`explain`, add `EngineBackend.fork()` = new engine from `{seed, transcript prefix}` (or engine-level serialize of state+internalState+`serializeFlowState()`), not `undo()`.
...
- `undo()`/`replay()` limitations above; `Date.now()`/`crypto.randomUUID()` in state/history; bots using `Math.random`.
