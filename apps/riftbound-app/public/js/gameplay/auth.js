...
(async function initAuthBar() {
  ...
  try {
    ...
    if (cr.ok) {
      ...
      if (creds && creds.token) { document.cookie = 'rb_token=' + creds.token + ';path=/;max-age=' + (30*86400) + ';SameSite=Lax'; try{localStorage.setItem('rb_token', creds.token);}catch{} if (typeof showUser==='function' && creds.user) { showUser(creds.user.displayName || creds.user.username); return; } if (location.pathname==='/login') { location.href='/'; return; } }
