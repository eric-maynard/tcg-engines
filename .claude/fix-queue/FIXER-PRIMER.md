...
- **Ruling vs Core Rules conflicts — JUDGE RULINGS WIN (product owner, 2026-08-13; this REPLACES the old "engine follows CR" rule).** "yeah follow the judge rulings." When a ruling and our reading of the Core Rules disagree, implement the RULING. The CR is the tiebreak in exactly three cases, and you must name which one you used: (1) **no ruling speaks** to the question; (2) **rulings disagree with each other** — including an official answer contradicting a community one — so there is no single judge answer to follow (cite BOTH ids); (3) the ruling **demonstrably describes superseded text** (pre-Unleashed rules, pre-errata card wording) **and Eric has confirmed it** — "demonstrably" means evidence, not a rules argument: text that no longer exists, a rule id in no shipped Core Rules version, or a later answer stating the change. Case 3 is NOT yours to settle: annotate, leave the engine alone, and surface the item to main/Eric. Do NOT flip the engine back and forth; a facet that does not follow its ruling keeps a `// RULING-CONFLICT: riftjudge <id> says X; <rule> says Y — tiebreak <1|2|3>, engine follows <what>` note plus a "previously asserted the opposite, do not flip back" line. Standing example of a mis-settled conflict: `hidden-blade-719c8ada539c1401` was filed as CR-beats-ruling in Aug 2026 by reading 359.3.e.5 without 359.3.e.14.b — the ruling and the engine had agreed all along. Full policy + the 2026-08-13 re-audit table: DESIGN.md § "Community rulings vs the CR". NOTE for anyone re-opening old settlements: three other primer sections still encode CR-wins settlements from before this policy (§2 optional-instruction timing ~28 train ids, §6 countered cards and "played", § BATTLEFIELD CONTROL TIMING) — they have NOT been re-audited; ask main before touching them.
...
- **Check `push_failed=` / `push_unverified=` before you believe a land shipped.** The gate used to pipe `git push` through `tail | sed`, which threw away the exit code — a FAILED push printed a fragment of the error as if it were a success line while the commit stayed local-only (this happened; a coordinator commit sat unpushed until someone noticed origin was behind). The gate now reports `push_failed=rc=N` and, on success, cross-checks `git ls-remote` against local HEAD and prints `push_unverified=` if origin does not match. If you see either line, re-run `git push origin <branch>` yourself — the commit is real but nobody else has it, and the devbox may be running code that is not on origin.
- **A verification worktree must resolve `@tcg/*` INTO ITSELF, or you are testing the shared tree.** Symlinking a fresh worktree's `node_modules` at the repo root (or per package) back to the shared checkout makes every `@tcg/...` import resolve to SHARED sources — so you run a clean checkout's TESTS against the shared tree's CODE and get failures that exist nowhere in git. This is how "HEAD is red" was reported by three separate lanes, and by the coordinator, when HEAD was green the whole time (32,118 tests / 0 fail). Set the workspace links so `@tcg/<pkg>` points at `<your-worktree>/packages/<pkg>`, then confirm before trusting a red: `bun -e 'console.log(require.resolve("@tcg/riftbound-engine"))'` must print a path inside YOUR worktree.
- **Before declaring HEAD red, run `.claude/fix-queue/stale-shadow-check.sh`.** The shared tree routinely holds pre-land copies that shadow HEAD. A stale copy is provable, not a judgement call: if the working bytes equal SOME earlier commit's blob for that path, it contains no local work and restoring it from HEAD is lossless. The post-land refresh now uses exactly that provenance test — the previous "adds nothing new" heuristic skipped a stale copy that REPLACED a line rather than only deleting some, which is precisely the case that shadowed a landed game-end guarantee and cost the fleet a stop-the-line investigation.
- **Run your land with the Bash sandbox DISABLED, and check the three network lines.** Inside an agent's sandbox `github.com:22` is a denied host and ssh's control socket is EPERM, so the gate's push, rsync and bounce all fail — and they fail quietly: `app=` comes back EMPTY (reads like "nothing to do" rather than "ssh could not run"), while the commit itself succeeds. So a land can report `committed=true` with the code on nobody's machine but this container's. After every land, read `push_failed=` / `push_unverified=`, `sync_failed=` and `bounce_failed=`; if any appears, re-run that step with `dangerouslyDisableSandbox: true` and then VERIFY on the box (`ssh emaynard-tcg 'grep -c <your-symbol> <path>'` and `curl /play` → 200) rather than trusting the retry. A commit that never reached origin is lost if the container is: `git ls-remote origin <branch>` against local HEAD is the check, and it takes a second.
- **`refs/remotes/origin/<branch>` missing does NOT mean origin lacks the branch.** This checkout has no fetch refspec creating remote-tracking refs for working branches, so `git for-each-ref refs/remotes/origin` shows only `main` and one other — three separate lanes concluded from that "the entire branch is local to this container" and escalated. It is not: check with `git ls-remote origin <branch>` and compare to `git rev-parse HEAD`. Lands from a sandboxed agent session genuinely cannot push (`github.com:22` is denied), so a `push_failed=` line IS real and the commit does need pushing — but the branch itself has been on origin all along, and a coordinator sweep pushes stranded commits every 10 minutes.
- **From an agent session, fire a land only while the lock is actually free.** A land that has to wait gets reaped mid-`flock` and prints NOTHING — not even `files=` — so it looks like a clean no-op rather than a failure (this ate several lands, including one that had already passed its suite). `land-when-free.sh` polls cheaply and only then hands off, which is the safe pattern; if you invoke `land-patch.sh` directly, check `flock -n .claude/fix-queue/.land.lock true` first and retry rather than parking inside it. Always confirm with `git log --oneline -3` before re-running, so a reaped-but-committed land is not landed twice.
- **A land survives only under `setsid` with the sandbox DISABLED.** A Bash tool call that hits its timeout has its whole process group killed, and a sandboxed call runs in its own PID namespace so it takes its children with it — that is why lands park in `flock` and vanish without printing even `files=`, leaving orphan tickets behind. The combination that sticks: `land-when-free.sh` (so you wait outside the gate and enter fairly) invoked under `setsid`, in a call with `dangerouslyDisableSandbox: true` (which also lets the push, rsync and bounce actually reach the network). Confirm afterwards with `git log --oneline -3` before retrying, and clear your ticket if you abandon an attempt.
- **Before removing an offer that a "bug" calls illegal, check whether another LANDED ruling requires that same offer under a different parameterisation.** Piercing Light looked like it enumerated a duplicate-target tuple `[front,front]`; at `repeatCount=1` that tuple is REQUIRED (execution 1 hits front, execution 2 hits front) by 820.2.a and by landed ruling 48f43ad476d48972, whose test casts exactly that for 2+2=4. Distinctness words — "another", "other" — bind WITHIN one execution; [Repeat] executions choose afresh. A one-line guard on the repeat tail-builder looked verified and broke six tests in three sibling suites. Same shape as the Deceiver/Reflection pair: two rulings look contradictory until you notice they are scoped to different steps. When a new test and an old test disagree, the new one is the more likely to be wrong — it has been run fewer times.
- **An embargo is a second pair of eyes, not just an obstacle.** From a fixer's seat it looks like something blocking a one-line change; in practice it forced review of a "verified" fix that was about to trade one ruling for another. Keep a line only while its lane is ACTIVELY editing the file, and remove it the moment that lane reports done — a stale embargo silently fails queue items with `embargoed_file=`, which is its own kind of waste.
- **The gate now refuses a patch that leaves the engine non-importing (`import_broken=`).** Before running the suite it imports the harness barrel in its clean worktree — two seconds, unambiguous. This exists because a commit landed whose `decision.ts` imported `hideCostQuote`/`instructionCost` from `hide.ts`/`play-pipeline.ts` while those files stayed uncommitted in the shared tree: HEAD then failed to import for everyone, so every subsequent land failed the gate and lanes lost time bisecting a break that was really a half-landed change. If you see `import_broken=`, you almost certainly listed only some of the files your change spans — add the siblings that define the symbols you import. The general rule: land a change WHOLE, and if you must split it, land the DEFINITIONS before the callers.
- **Do not close an item until the gate says `committed=true` for the files carrying the fix.** The biggest single leak found in this loop was not wrong fixes — it was fixes that never landed: a lane verifies in its own worktree, marks the item done, and its land is refused or reaped, so the item reads as finished forever while the marker sits in the tree still failing. A triage found SEVEN at once. Verify with `git log --oneline -3` (or `git show HEAD:<file> | grep <your symbol>`) before writing the item to `done/`, and land the FIX and the MARKER FLIP together — a fix whose flip did not land is indistinguishable from no fix. Audit the whole set any time with `python3 .claude/fix-queue/done-but-not-landed.py` (add `--reopen` to reopen offenders); it matches each item's own words against a still-failing marker at HEAD rather than merely asking whether the file contains one, and it prints `scanned=N` because a silent all-clear is not evidence.
- **`.land.priority` and `.land.fast-waiting` both starve the fleet when a lane forgets to clean up.** A single line naming a finished lane makes every other label sleep 20s per poll for up to 20 minutes, and it looks exactly like lock contention — a land prints `files=N` and then nothing. Both are now self-expiring (a priority list older than `LAND_PRIORITY_TTL`, default 1h, is cleared with `reaped_stale_priority=`; yield markers age out at `LAND_MARKER_TTL`), but remove your own line the moment your land commits. If a land stalls silently after `files=`, look at those two files FIRST — `cat .claude/fix-queue/.land.priority` and `ls .claude/fix-queue/.land.fast-waiting.*` — before suspecting the lock.
