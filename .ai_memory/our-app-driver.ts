...
// C0 lobby
await shot("ours-00-lobby"); await dump("ours-00-lobby");
...
for (let i = 0; i < 60; i++) {
  ...
  if (p.pregame === "battlefield_select" && !dumped.has("bf")) { await shot("ours-01-battlefield-select"); await dump("ours-01-battlefield-select"); dumped.add("bf"); }
  if (p.pregame === "mulligan" && !dumped.has("mull")) { await shot("ours-02-mulligan"); await dump("ours-02-mulligan"); dumped.add("mull"); }
...
await page.waitForTimeout(1000);
await shot("ours-03-turn1-main"); await dump("ours-03-turn1-main");
...
if (played) { await page.waitForTimeout(1500); await shot("ours-04-after-first-play"); await dump("ours-04-after-first-play"); }
...
if ((await endTurn.count()) && await endTurn.isVisible().catch(()=>false)) {
  await endTurn.click(); await page.waitForTimeout(1500); await shot("ours-08-endphase"); await dump("ours-08-endphase");
}
