...
await shot("riftatlas-tick2-00-home");
...
for (let i = 0; i < 50; i++) {
  ...
  if (txt.includes("CHOOSE BATTLEFIELD")) {
    if (!bfShot) { await shot("riftatlas-tick2-22-choose-bf"); await dump("riftatlas-tick2-22-choose-bf"); bfShot = true; }
...
await shot(reachedMatch ? "riftatlas-tick2-30-turn1-main" : "riftatlas-tick2-30-stuck");
await dump(reachedMatch ? "riftatlas-tick2-30-turn1-main" : "riftatlas-tick2-30-stuck");

if (reachedMatch) {
  // Capture various in-match states
  await shot("riftatlas-tick2-31-board"); await dump("riftatlas-tick2-31-board");
  // Hover a hand card (bottom-center band) for preview
  await page.mouse.move(W*0.5, H*0.92); await page.waitForTimeout(1200);
  await shot("riftatlas-tick2-32-hand-hover");
  // Click DRAW (D key) to get a card, then screenshot
  await page.keyboard.press("d"); await page.waitForTimeout(1200);
  await shot("riftatlas-tick2-33-after-draw"); await dump("riftatlas-tick2-33-after-draw");
  // Click a hand card
  await page.evaluate(() => {
  ...
  await page.waitForTimeout(1500);
  await shot("riftatlas-tick2-34-hand-card-clicked"); await dump("riftatlas-tick2-34-hand-card-clicked");
  // Space to advance phase
  await page.keyboard.press("Space"); await page.waitForTimeout(1500);
  await shot("riftatlas-tick2-35-after-space");
  ...
  await shot("riftatlas-tick2-36-chain-panel");
  ...
  await page.waitForTimeout(1200);
  await shot("riftatlas-tick2-37-help-modal");
}
