...
// C0 lobby
await shot("ours-tick3-00-lobby"); await dump("ours-tick3-00-lobby");
...
for (let i = 0; i < 60; i++) {
  ...
  if (p.pregame === "battlefield_select" && !dumped.has("bf")) { await shot("ours-tick3-01-battlefield-select"); await dump("ours-tick3-01-battlefield-select"); dumped.add("bf"); }
  if (p.pregame === "mulligan" && !dumped.has("mull")) { await shot("ours-tick3-02-mulligan"); await dump("ours-tick3-02-mulligan"); dumped.add("mull"); }
...
await page.waitForTimeout(1000);
await shot("ours-tick3-03-turn1-main"); await dump("ours-tick3-03-turn1-main");
...
if (played) { await page.waitForTimeout(1500); await shot("ours-tick3-04-after-first-play"); await dump("ours-tick3-04-after-first-play"); }
...
if ((await endTurn.count()) && await endTurn.isVisible().catch(()=>false)) {
  await endTurn.click(); await page.waitForTimeout(1500); await shot("ours-tick3-08-endphase"); await dump("ours-tick3-08-endphase");
}

await Bun.write(`${OUT}/ours-tick3-console.log`, logs.join("\n"));
await b.close();
console.log("done");
